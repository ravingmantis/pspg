...
pspg_call <- function(tbl, args, row.names = FALSE, col.names = TRUE) {
  if (getOption("pspg.args.ignore-case", default = TRUE)) args[["ignore-case"]] <- TRUE
  if (isTRUE(row.names)) args[["freezecols"]] <- "0"
  if (isTRUE(col.names)) args[["csv-header"]] <- "on"

  arg_strings <- vapply(unique(names(args)), function (n) {
    v <- args[[n]]
    if (isTRUE(v)) return(paste0("--", n))
    return(paste0("--", n, "=", v))
  }, character(1))
  ...
  utils::write.csv(tbl, file = tmp_path, row.names = row.names, col.names = col.names)
...
pspg.list <- function (x, ...) {
  tbl <- data.frame(
    ...
    value = vapply(x, function(v) paste(as.character(v), collapse = ", "), character(1)),
